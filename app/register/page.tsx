      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <motion.div className="w-full max-w-sm" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }}>
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            <div>
              ...
              <input type="email" value={email} onChange={e => setEmail(e.target.value)} required autoComplete="email"
                ...
                className="w-full px-4 py-3 rounded-xl border border-border bg-surface text-primary text-sm
                  focus:outline-none focus:ring-2 focus:ring-accent/30 focus:border-accent/50 transition-all placeholder:text-muted" />
            </div>
              ...
              <div className="flex gap-1">
                {[1,2,3,4].map(i => (
                  <div key={i} className={`h-1 flex-1 rounded-full transition-all duration-300 ${
                    password.length >= i * 3 ? i <= 2 ? 'bg-accent/60' : i === 3 ? 'bg-accent' : 'bg-green-500' : 'bg-border'
                  }`} />
                ))}
              </div>
            )}
            ...
            {error && (
              <motion.p className="text-danger text-sm bg-danger/10 border border-danger/20 px-4 py-2.5 rounded-xl"
            ...
            <button type="submit" disabled={loading}
              className="w-full py-3 bg-accent text-white rounded-xl font-semibold text-sm
                ...
                transition-all hover:scale-[1.01] active:scale-[0.99] flex items-center justify-center gap-2 mt-2">
          ...
          <p className="text-center text-sm text-muted mt-6">
            Đã có tài khoản?{' '}
            <Link href="/login" className="text-accent hover:text-primary font-semibold transition-colors">Đăng nhập</Link>
          </p>
        </motion.div>
      </div>
    </div>
